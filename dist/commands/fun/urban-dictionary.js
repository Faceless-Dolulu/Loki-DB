...
export async function run({ interaction, client, handler }) {
    try {
        ...
        const urbanDictionaryResult = await request(`https://api.urbandictionary.com/v0/define?${query}`);
        //@ts-ignore
        const { list } = await urbanDictionaryResult.body.json();
        const channel = interaction.channel;
        if (!channel.nsfw) {
            interaction.followUp("Sorry, Urban Dictionary can break Discord TOS. Please try using this command again inside an NSFW channel. 🙃 ");
            return;
        }
        if (!list.length) {
        ...
        for (let i = 0; i <= 4; i++) {
            ...
            const newEmbed = new EmbedBuilder()
                ...
                .addFields({ name: "Defintion", value: `${trim(list[i].definition, 1024)}` }, { name: `Example`, value: `${trim(list[i].example, 1024)}` }, {
                name: "Rating",
        ...
        interaction.followUp("An error occured. Please try again later. If the error persists, please make a ticket in the Support server.");
    }
}
