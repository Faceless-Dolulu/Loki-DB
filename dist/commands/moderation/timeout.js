...
export async function run({ interaction, client, handler }) {
    try {
        ...
        const timeoutLogMessage = new EmbedBuilder()
            ...
            .addFields({ name: '\u200b', value: `:mute: **Timed Out** ${targetUser.user.username} (ID ${targetUser.user.id})` }, { name: '\u200b', value: `:page_facing_up: **Reason:** ${reason}` }, { name: '\u200b', value: `:stopwatch: **Duration:** ${prettyMilliseconds(msDuration, { verbose: true })}` })
            .setTimestamp();
        ...
        for (const timeoutConfig of timeoutConfigs) {
            const timeoutLogChannel = targetUser.guild.channels.cache.get(timeoutConfig.channelId)
                ||
                    (await targetUser.guild.channels.fetch(timeoutConfig.channelId));
