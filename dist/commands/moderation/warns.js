...
export const data = new SlashCommandBuilder()
    .setName("warns")
    .setDescription("Add/View/Edit/Delete/Clear warns of a particular user in this server.")
    ...
    try {
        ...
        if (subCommand === "add") {
            ...
            if (targetUser.id === executor.id) {
                await interaction.followUp(`❌ You can't issue a warn against yourself!`);
                return;
            }
            if (!targetMember) {
            ...
            if (targetRolePosition >= executorRolePosition) {
                await interaction.followUp(`❌ You can't issue a warn to a user with equal or higher roles!`);
                return;
            }
            ...
            newWarn.save().then(() => {
                console.log(`A new warning for ${targetUser.username} has been issued in ${targetMember.guild.name}`);
            });
            const warnConfigs = await warnLogsChannelSchema.find({
                guildId: targetMember.guild.id,
            });
            for (const warnConfig of warnConfigs) {
                const warnLogChannel = interaction.guild?.channels.cache.get(warnConfig.channelId) ||
                    (await interaction.guild?.channels.fetch(warnConfig.channelId));
                ...
                interaction.followUp(`:tools: ${targetUser} has been warned | ${reason}`);
            }
        }
        if (subCommand === "view") {
            const targetMember = interaction.options.getMember("user");
            ...
            if (warnsArray.length === 0) {
                interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setTimestamp()
                            .setTitle(`Warnings - User: ${targetMember.user.username} (ID ${targetMember.id})`)
        ...
        if (subCommand === "delete") {
            ...
            for (const warnConfig of warnConfigs) {
                const adminLogChannel = interaction.guild?.channels.cache.get(warnConfig.channelId) ||
                    (await interaction.guild?.channels.fetch(warnConfig.channelId));
        ...
        if (subCommand === "clear") {
            ...
            if (!warnsExistInDb) {
                interaction.followUp(`❌ This does not have a warn history in this server!`);
                return;
            }
            ...
            interaction.followUp(`All warns for ${targetMember.user.username} have been deleted!`);
            for (const warnConfig of warnConfigs) {
                const adminLogChannel = interaction.guild?.channels.cache.get(warnConfig.channelId) ||
                    (await interaction.guild?.channels.fetch(warnConfig.channelId));
