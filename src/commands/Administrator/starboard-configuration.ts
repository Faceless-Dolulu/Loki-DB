		...
		command
			...
			.addIntegerOption((option) =>
				option
					...
					.setDescription(
						"The minimum amount of reactions needed to append a message to the starboard. Default is 5"
					)
			)
			.addStringOption((option) =>
				option
					.setName("emoji")
					.setDescription(
						"Set what emoji you want to use for your starboard. Default is ⭐"
					)
			)
	)
	.addSubcommand((command) =>
...
export async function run({ interaction, client, handler }: SlashCommandProps) {
	try {
		...
		if (subCommand === "add") {
			...
			const minReactions =
				interaction.options.getInteger("reaction-count") || 5;
			const query = {
			...
			if (channelExistsInDb) {
				interaction.followUp(
					"This server already has a starboard channel configured!"
				);
				return;
			}
