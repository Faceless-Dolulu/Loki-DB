				...
				subcommand
					...
					.addChannelOption((option) =>
						option
							.setName("channel")
							.setDescription(
								"The channel to remove from logging timeout events."
							)
							.addChannelTypes(ChannelType.GuildText)
							.setRequired(true)
					)
		...
		subcommandGroup
			...
			.addSubcommand((subcommand) =>
				subcommand
					.setName("add")
					.setDescription(
						"Add a channel to log message events. (Deletions and edits)"
					)
					.addChannelOption((option) =>
						option
							.setName("channel")
							.setDescription(
								"The channel to host delete and edit message logs."
							)
							.addChannelTypes(ChannelType.GuildText)
							.setRequired(true)
					)
			...
			.addSubcommand((subcommand) =>
				subcommand
					.setName("remove")
					.setDescription(
						"Remove a channel from logging delete and edit message events."
					)
					.addChannelOption((option) =>
						option
							.setName("channel")
							.setDescription(
								"The channel to remove from logging delete and edit message events."
							)
							.addChannelTypes(ChannelType.GuildText)
							.setRequired(true)
					)
				...
				subcommand
					...
					.addChannelOption((option) =>
						option
							.setName("channel")
							.setDescription(
								"The channel to remove from logging issued warns."
							)
							.addChannelTypes(ChannelType.GuildText)
							.setRequired(true)
					)
	...
	try {
		...
		if (subCommandGroup === "bans") {
			if (subCommand === "add") {
				...
				if (channelExistsInDB) {
					interaction.followUp(
						"This channel has already been configured for ban logs."
					);
					return;
				}
				...
				newBanChannel
					...
					.then(() => {
						interaction.followUp(
							`Configured ${banLogChannel} to host ban logs.`
						);
					})
					.catch((error) => {
						interaction.followUp(
			...
			if (subCommand === "remove") {
				...
				if (!channelExistsInDb) {
					interaction.followUp(
						"That channel has not been configured for ban logs!"
					);
					return;
				}
				...
				banLogsChannelSchema
					...
					.then(() => {
						interaction.followUp(
							`Removed ${banLogChannel} from hosting ban logs.`
						);
					})
					.catch((error) => {
						interaction.followUp(
							`Databse error. Please try again in a moment.`
						);
						console.log(
							`DB error in ${fileURLToPath(import.meta.url)}:\n`,
							error
		...
		if (subCommandGroup === "kicks") {
			if (subCommand === "add") {
				...
				if (channelExistsInDb) {
					interaction.followUp(
						"This server has already been configured with a kick logs channel"
					);
					return;
				}
				...
				newKickChannel
					...
					.then(() => {
						interaction.followUp(
							`Configured ${kickLogChannel} to host kick logs.`
						);
					})
					.catch((error) => {
						interaction.followUp(
			...
			if (subCommand === "remove") {
				...
				if (!channelExistsInDb) {
					interaction.followUp(
						`This server doesn't have a channel configured for ban logs!`
					);
					return;
				}
				...
				kickLogsChannelSchema
					...
					.then(() => {
						interaction.followUp(
							`Removed ${kickLogChannel} from hosting kick logs.`
						);
					})
					.catch((error) => {
						interaction.followUp(
		...
		if (subCommandGroup === "timeouts") {
			if (subCommand === "add") {
				...
				if (channelExistsInDb) {
					interaction.followUp(
						"This channel has already been configured for timeout logs."
					);
					return;
				}
				...
				newTimeoutChannel
					...
					.then(() => {
						interaction.followUp(
							`Configured ${timeoutLogChannel} to host timeout logs.`
						);
					})
					.catch((error) => {
						interaction.followUp(
			...
			if (subCommand === "remove") {
				...
				if (!channelExistsInDb) {
					interaction.followUp(
						"That channel has not been configured to host timeout logs!"
					);
					return;
				}
				...
				timeoutLogsChannelSchema
					...
					.then(() => {
						interaction.followUp(
							`Removed ${timeoutLogChannel} from hosting timout logs`
						);
					})
					.catch((error) => {
		...
		if (subCommandGroup === "messages") {
			if (subCommand === "add") {
				...
				if (channelExistsInDb) {
					interaction.followUp(
						"This channel has already been configured for logging message events."
					);
					return;
				}
				...
				newLogChannel
					...
					.then(() => {
						interaction.followUp(
							`Configured ${logChannel} to log message events.`
						);
					})
					.catch((error) => {
						interaction.followUp(
			...
			if (subCommand === "remove") {
				...
				if (!channelExistsInDB) {
					interaction.followUp(
						"This server doesn't have a channel configured to log message events!"
					);
					return;
				}
				...
				messageLogsChannelSchema
					...
					.then(() => {
						interaction.followUp(
							`Removed ${logChannel} from logging message events.`
						);
					})
					.catch((error) => {
						interaction.followUp(
		...
		if (subCommandGroup === "warns") {
			if (subCommand === "add") {
				...
				if (channelExistsInDb) {
					interaction.followUp(
						"This channel has already been configured for logging issued warns."
					);
					return;
				}
				...
				newWarnChannel
					...
					.then(() => {
						interaction.followUp(
							`Configured ${warnLogChannel} to log issued warns.`
						);
					})
					.catch((error) => {
						interaction.followUp(
						...
						console.log(
							`DB error when adding warn channel in ${fileURLToPath(
								import.meta.url
							)}:\n`,
							error
						);
			...
			if (subCommand === "remove") {
				...
				if (!channelExistsInDb) {
					interaction.followUp(
						`That channel has not been configured for logging warns!`
					);
					return;
				}
				...
				banLogsChannelSchema
					...
					.then(() => {
						interaction.followUp(
							`Removed ${warnLogChannel} from hosting warn logs.`
						);
					})
					.catch((error) => {
						interaction.followUp(
		...
		if (subCommandGroup === "suggestions") {
			if (subCommand === "enable") {
				...
				if (channelExistsInDb) {
					interaction.followUp(
						"This server already has a suggestions system configured"
					);
					return;
				}
				...
				newSuggestionChannel
					...
					.then(() => {
						interaction.followUp(
							`Suggestions have now been configured. When a suggestion is made, it will be sent to ${suggestionChannel}`
						);
					})
					.catch((error) => {
						interaction.followUp(
			...
			if (subCommand === "disable") {
				...
				if (!suggestionConfigured) {
					interaction.followUp(
						`This server doesn't have suggestions configured for this server.`
					);
					return;
				}
				...
				suggestionChannelSchema
					...
					.then(() => {
						interaction.followUp(
							`Suggestions have now been disabled in this server.`
						);
					})
					.catch((error) => {
						interaction.followUp(
